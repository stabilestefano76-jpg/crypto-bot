...
export default function Top10Screen() {
  ...
  const load = useCallback(async () => {
    try {
      const p = await top10Api.portfolio();
      setPortfolio(p);
    } catch {
      // ignore
    } finally {
  ...
  const onWithdraw = async () => {
    ...
    setWithdrawing(true);
    try {
      await top10Api.withdraw(amount);
  ...
  const renderPosition = (p: Top10Position, closed: boolean) => {
    ...
    return (
      <View key={p.id} style={styles.posCard}>
        ...
        <Text style={styles.posMeta}>
          SL {p.current_stop.toFixed(4)}  ·  TP1 {p.tp1.toFixed(4)}  ·  TP2 {p.tp2.toFixed(4)}
        </Text>
        {!closed && (p.tp1_hit || p.tp2_hit) && (
          <Text style={styles.posMeta}>
            {p.tp1_hit ? "TP1 raggiunto" : ""}{p.tp1_hit && p.tp2_hit ? " · " : ""}
            {p.tp2_hit ? "TP2 raggiunto, resto in trailing" : ""}
          </Text>
        )}
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Top 10 Long</Text>
        <View style={{ width: 26 }} />
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalSubtitle}>
              Sposta fondi dal portafoglio principale a Top 10 Long.
            </Text>
            <TextInput
      ...
      <Modal visible={withdrawVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalSubtitle}>
              Sposta fondi da Top 10 Long al portafoglio principale.
            </Text>
            <TextInput
              style={styles.modalInput}
              placeholder="Importo in USDT"
      ...
      <Modal visible={resetVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Reset Top 10 Long</Text>
            <Text style={styles.modalSubtitle}>
