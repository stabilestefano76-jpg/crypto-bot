...
export default function GridScreen() {
  ...
  const activeInstances = useMemo(
    () => instances.filter((g) => g.status === "active"),
    [instances]
  );

  const selected = useMemo(
    () => activeInstances.find((g) => g.symbol === selectedSymbol) ?? activeInstances[0] ?? null,
    [activeInstances, selectedSymbol]
  );

  const load = useCallback(async () => {
    try {
      const [p, i] = await Promise.all([gridWalletApi.portfolio(), gridApi.instances()]);
      setPortfolio(p);
  ...
  const onTransfer = async () => {
    ...
    try {
      if (transferMode === "deposit") {
        await gridWalletApi.transfer(amount);
      } else {
        await gridWithdraw(amount);
      }
  ...
  const onReset = () => {
    Alert.alert(
      "Reset Grid Bot",
      "Verranno chiuse tutte le griglie attive, cancellato lo storico e azzerato il portafoglio griglia (il saldo attuale andrà perso). Il portafoglio principale non viene toccato. Continuare?",
      [
  ...
  const renderPosition = (p: GridPosition, closed: boolean) => {
    ...
    return (
      <View key={p.id} style={styles.posCard}>
        ...
        <Text style={styles.posMeta}>Target {p.target.toFixed(6)}</Text>
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Grid Bot</Text>
      ...
      {loading && !portfolio ? (
        ...
        >
          {activeInstances.length > 1 && (
            ...
            >
              {activeInstances.map((g) => (
                <Pressable
                  key={g.symbol}
                  onPress={() => setSelectedSymbol(g.symbol)}
                  style={[
                    styles.chip,
                    g.symbol === selected?.symbol && styles.chipActive,
                  ]}
                >
                  <Text
                    style={[
                      styles.chipText,
                      g.symbol === selected?.symbol && styles.chipTextActive,
                    ]}
                  >
                    {g.symbol}
                  </Text>
          ...
          {selected ? (
            <GridChart
              ...
              levels={{
                ...
                centerPrice: selected.center_price,
              }}
              height={260}
            />
          ) : (
            <View style={styles.emptyChart}>
              <Text style={styles.emptyText}>
                Nessuna griglia attiva al momento. Il bot sta cercando un mercato laterale
                adatto tra le coppie a volume più alto.
              </Text>
            </View>
          )}
          ...
          <View style={styles.walletCard}>
            ...
            <View style={styles.walletRow}>
              <View style={styles.walletStat}>
                <Text style={styles.walletLabel}>P&L operazioni chiuse</Text>
                <Text
                  style={[
                    styles.walletValue,
                    {
            ...
            >
              ...
              <Text style={styles.resetBtnText}>Reset Grid Bot</Text>
            </Pressable>
          </View>
          ...
          {(portfolio?.open_positions ?? []).length === 0 ? (
            <Text style={styles.emptyText}>Nessuna cella in possesso al momento.</Text>
          ) : (
          ...
          <View style={styles.historyHeaderRow}>
            <Text style={styles.sectionTitle}>
              Storico recente ({portfolio?.closed_count ?? 0}) · Win rate{" "}
              {portfolio?.win_rate ?? 0}%
            </Text>
            <Pressable onPress={() => router.push("/grid-history")} hitSlop={8}>
          ...
          {(portfolio?.closed_positions ?? []).length === 0 ? (
            ...
            portfolio!.closed_positions.slice(0, 10).map((p) => renderPosition(p, true))
          )}
        </ScrollView>
      )}
          ...
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalSubtitle}>
              {transferMode === "deposit"
                ? "Sposta fondi dal portafoglio principale a quello del Grid Bot."
                : "Riporta fondi dal Grid Bot al portafoglio principale."}
            </Text>
            <TextInput
