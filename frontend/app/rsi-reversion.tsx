...
export default function RsiReversionScreen() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { panHandlers, index, total } = useSwipeNavigation("/rsi-reversion");
  ...
  const load = useCallback(async () => {
    try {
      const p = await rsiReversionApi.portfolio();
      setPortfolio(p);
    } catch {
      // ignore
    } finally {
  ...
  const onDeposit = async () => {
    ...
    setDepositing(true);
    try {
      await rsiReversionApi.deposit(amount);
  ...
  const onWithdraw = async () => {
    ...
    setWithdrawing(true);
    try {
      await rsiReversionApi.withdraw(amount);
    ...
    return (
      <View key={p.id} style={styles.posCard}>
        <View style={styles.posTop}>
          ...
          >
            <Text style={styles.setupBadgeText}>{p.side === "long" ? "LONG" : "SHORT"}</Text>
          </View>
        </View>
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]} {...panHandlers}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>RSI Reversion</Text>
        <View style={{ width: 26 }} />
      </View>
      <Text style={styles.subtitle}>
        RSI in ipercomprato/ipervenduto, rientro con divergenza confermata
      </Text>
      ...
      {loading && !portfolio ? (
        ...
        >
          <View style={styles.walletCard}>
            {portfolio?.wallet_type === "shared" && (
              <Text style={styles.sharedNotice}>
                Ancora sul portafoglio condiviso principale — deposita un importo per isolarlo.
              </Text>
            )}
          ...
          {(portfolio?.closed_trades ?? []).length === 0 ? (
            ...
            portfolio!.closed_trades.slice(0, 30).map((p) => renderPosition(p, true))
          )}
        </ScrollView>
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalSubtitle}>
              Sposta fondi dal portafoglio principale a RSI Reversion.
            </Text>
            <TextInput
      ...
      <Modal visible={withdrawVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalSubtitle}>
              Sposta fondi da RSI Reversion al portafoglio principale.
            </Text>
            <TextInput
              style={styles.modalInput}
              placeholder="Importo in USDT"
      ...
      <Modal visible={resetVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Reset RSI Reversion</Text>
            <Text style={styles.modalSubtitle}>
